...
pozadinski_zvuk = pygame.mixer.music.load ("pozadinski.mp3")  #pozadinska muzika
...
rijec_font = pygame.font.SysFont('dejavusansmono', 45)       #font i veličina rijeci
...
while run:          #dok je god varijabla run true, petlja se izvrsava
            ...
            for slovo in slova:
                ...
                if vidljivo:
                    ...
                    if dis < RADIUS:
                        ...
                        if ltr in riječ:
                            pogodak = mixer.Sound('pogodak.wav')  #inicijalizacija zvuka pogotka
                            pogodak.play() #puštanje zvuka pogotka
                        if ltr not in riječ: #ako odabrano slovo nije u traženoj riječi
                            kiks = mixer.Sound('pogresno_slovo.wav')  #inicijalizacija zvuka pogreške
    ...
    if pobjeda:
        win = mixer.Sound('win.wav')  #inicijalizacija zvuka pobjede
        win.play() #puštanje zvuka pobjede
        ispis_poruke("Pobjeda!") #metoda s parametrom pobjeda
        break

    if hangman_status == 6:
        gameover = mixer.Sound('gameover.wav')  #inicijalizacija gameover zvuka   
